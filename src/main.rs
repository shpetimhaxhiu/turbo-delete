...
fn main() {
    ...
    // Process all provided paths (support for multiple selection)
    let paths_to_delete: Vec<String> = args[1..].iter().map(|arg| {
...
fn delete_path(file_path: &str) {
    ...
    if !path.exists() {
        eprintln!(
            "{} {}",
            " WARNING ".on_color(AnsiColors::BrightYellow).black(),
            format!("Path does not exist: {}", file_path).bright_yellow()
        );
        return;
    }
